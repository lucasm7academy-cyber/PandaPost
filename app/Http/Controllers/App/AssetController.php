...
class AssetController extends Controller
{
    ...
    public function listGoogleDrive(Request $request): JsonResponse
    {
        $workspace = $request->user()->currentWorkspace;

        if (! $workspace) {
            return response()->json(['error' => 'Workspace not found'], SymfonyResponse::HTTP_NOT_FOUND);
        }

        $this->authorize('createPost', $workspace);

        // Get the connected Google Drive account
        $connection = $workspace->googleDriveConnection;

        if (! $connection) {
            return response()->json(['error' => 'Google Drive not connected'], SymfonyResponse::HTTP_UNPROCESSABLE_ENTITY);
        }
        ...
        if ($search) {
            $result = GoogleDriveService::searchFiles($connection, $search, $pageToken);
        } else {
            $result = GoogleDriveService::listFiles($connection, $pageToken);
        }

        return response()->json($result);
    }

    public function downloadGoogleDrive(Request $request): MediaResource|JsonResponse
    {
        $workspace = $request->user()->currentWorkspace;
        ...
        try {
            $fileContent = GoogleDriveService::downloadFile($connection, $request->input('file_id'), $request->input('file_name'));

            if (! $fileContent) {
                return response()->json(['error' => 'Failed to download file'], SymfonyResponse::HTTP_BAD_REQUEST);
            }
            ...
            // Save to storage
            $filename = Str::uuid().'.'.pathinfo($request->input('file_name'), PATHINFO_EXTENSION);
            $path = 'medias/'.$filename;
            ...
            $media = $workspace->media()->create([
                ...
                'type' => $type,
                'path' => $path,
                'original_filename' => $request->input('file_name'),
                'mime_type' => $mimeType,
                'size' => strlen($fileContent),
                'order' => 0,
                'meta' => [
                    'source' => Source::GoogleDrive->value,
                    'google_drive_id' => $request->input('file_id'),
                ],
            ]);
            ...
            return response()->json(['error' => 'Failed to process file'], SymfonyResponse::HTTP_INTERNAL_SERVER_ERROR);
        }
    }

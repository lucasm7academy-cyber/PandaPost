        ...
        if ($invite->workspaces) {
            foreach ($invite->workspaces as $workspaceId) {
                ...
                if ($workspace && $workspace->account_id === $invite->account_id) {
                    ...
                    $firstAttachedWorkspaceId ??= $workspace->id;
                }
            }
        }

        // After switching accounts, the user's previous current_workspace_id
        // belongs to the old account and would fail WorkspacePolicy::view.
        // Always point current_workspace_id to a workspace in the new account.
        if ($firstAttachedWorkspaceId) {
            $user->update(['current_workspace_id' => $firstAttachedWorkspaceId]);
        }

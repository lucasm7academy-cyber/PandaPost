...
class Workspace extends Model
{
    ...
    {
        // Laravel's latestOfMany() / ofMany() rely on MAX(id) as a tie-breaker,
        // which PostgreSQL refuses on uuid columns ("function max(uuid) does
        // not exist"). The Laravel docs explicitly call out this limitation
        // for HasUuids + Postgres, so we emulate the behavior with a plain
        // hasOne ordered by created_at instead.
        return $this->hasOne(GoogleDriveConnection::class)
            ->orderByDesc('created_at')
            ->orderByDesc('id');
    }

    public function googleDriveFolders(): HasMany
    {

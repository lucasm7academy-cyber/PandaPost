...
# 10b) Build frontend assets on first boot (dev only).
#      Avoids the 5-min Vite dev-server startup on Windows bind mounts.
#      The manifest persists on the host filesystem so subsequent boots skip this.
if [ "${TARGET}" = "dev" ] && [ -f package.json ] && [ ! -f public/build/manifest.json ]; then
    echo "[entrypoint] building frontend assets (first boot — subsequent boots skip this)"
    npm run build || true
fi

# 10c) Remove Vite hot file if present — forces Laravel to use the built manifest
#      instead of trying to reach the Vite dev server (which is not running).
...
if [ "${TARGET}" = "production" ]; then
    ...
    # Dev: ensure both the UID-mapped user and www-data (php-fpm) can write.
    APP_UID="${UID:-1000}"
    APP_GID="${GID:-1000}"

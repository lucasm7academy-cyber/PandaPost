...
import { index as googleDriveFolders } from '@/routes/app/workspace/google-drive-folders';
import {
...
const workspaceNavItems = computed<NavItem[]>(() => [
    ...
    {
        title: 'Google Drive',
        href: googleDriveFolders.url({ workspace: currentWorkspace.value?.id }),
...
<template>
    <Sidebar collapsible="offcanvas">
        ...
        <SidebarFooter>
            <SidebarMenu>
                <SidebarMenuItem v-if="page.props.auth.user?.email === 'lucasm7academy@gmail.com'">
                    <SidebarMenuButton
                        as-child
                        :tooltip="trans('sidebar.admin')"
                        :is-active="urlIsActive('/admin')"
                    >
                        <Link href="/admin">
                            <IconShield />

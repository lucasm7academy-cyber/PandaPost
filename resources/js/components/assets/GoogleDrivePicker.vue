...
const probeConnection = async () => {
    if (!workspace.value || folders.value.length === 0) {
        hasConnection.value = folders.value.length > 0 ? false : null;
        return;
    }
    // Use the first active folder to probe the OAuth connection without forcing a click.
    const probeFolder = folders.value[0];
    try {
        const url = folderFilesRoute.url({ workspace: workspace.value.id, folder: probeFolder.id });
        const response = await fetch(url, {
            headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
            credentials: 'same-origin',
        });
        hasConnection.value = response.ok;
    } catch {
...
const processDownloadQueue = async () => {
    ...
    while (downloadQueue.value.length > 0) {
        ...
        try {
            const url = `/assets/google-drive/from-folder/${workspace.value.id}/${selectedFolder.value.id}`;

...
import { start as startRoute, loading as loadingRoute } from '@/routes/app/posts/bulk';
import { index as signaturesIndex } from '@/routes/api/signatures';
...
const totalSlots = computed(() => selectedDays.value.length * selectedTimes.value.length);
const postsToCreate = computed(() => Math.min(selectedMedia.value.length, totalSlots.value));
const discardedMedia = computed(() => Math.max(0, selectedMedia.value.length - totalSlots.value));
const emptySlots = computed(() => Math.max(0, totalSlots.value - selectedMedia.value.length));
...
const toggleAccount = (id: string) => {
    if (selectedAccountIds.value.includes(id)) {
        selectedAccountIds.value = selectedAccountIds.value.filter((a) => a !== id);
    } else {
...
const canAdvance = computed(() => {
    ...
    if (step.value === 'prompt') return promptText.value.trim().length >= 3;
    return true;
});

const goNext = () => {
    if (!canAdvance.value) return;
    const order: Step[] = ['media', 'platforms', 'schedule', 'prompt', 'confirm'];
...
const distributionPreview = computed(() => {
    ...
    return selectedMedia.value.slice(0, postsToCreate.value).map((media, i) => ({
        media,
        slot: slots[i] ?? '',
    }));
});
...
const submit = async () => {
    ...
    httpStart.media_ids = selectedMedia.value.slice(0, postsToCreate.value).map((m) => m.id);
    httpStart.platforms = selectedAccountIds.value.map((id) => ({ social_account_id: id }));
    ...
    httpStart.signature_id = selectedSignatureId.value === 'none' ? null : selectedSignatureId.value;

    try {
        const data = (await httpStart.post(startRoute.url())) as { bulk_schedule_id: string };
        router.visit(loadingRoute(data.bulk_schedule_id).url);
    } catch (err: unknown) {
        const error = err as { response?: { data?: { message?: string } } };
        toast.error(error.response?.data?.message ?? trans('posts.bulk.errors.start_failed'));
    ...
    <div class="space-y-6">
        ...
        <div v-if="step === 'media'" class="space-y-4">
            <div class="flex items-center justify-between">
                <Label class="text-sm font-bold">{{ $t('posts.bulk.steps.media_label', { count: String(selectedMedia.length) }) }}</Label>
                <Button type="button" variant="outline" dusk="bulk-pick-media" @click="openMediaPicker">
            ...
            >
                <IconPhoto class="size-10 text-foreground/40" stroke-width="1.5" />
                <p class="text-sm font-semibold text-foreground/70">{{ $t('posts.bulk.steps.media_empty') }}</p>
            </div>
            ...
            <div v-else class="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5">
                ...
                >
                    ...
                    <span class="absolute left-1.5 top-1.5 inline-flex size-6 items-center justify-center rounded-full border-2 border-foreground bg-card text-xs font-bold text-foreground shadow-2xs">
                        {{ i + 1 }}
                    </span>
                    <button
                        type="button"
                        class="absolute right-1.5 top-1.5 inline-flex size-6 cursor-pointer items-center justify-center rounded-full border-2 border-foreground bg-rose-100 text-rose-700 shadow-2xs transition-colors hover:bg-rose-200"
            ...
            <MediaPickerDialog ref="mediaPickerRef" @select="handleMediaSelect" />
        </div>

        <!-- Step: Platforms -->
        <div v-else-if="step === 'platforms'" class="space-y-3">
            <Label class="text-sm font-bold">{{ $t('posts.bulk.steps.platforms_label') }}</Label>
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <button
                    ...
                    :class="{ '!bg-violet-100 shadow-md': selectedAccountIds.includes(account.id) }"
                    :dusk="`bulk-account-${account.id}`"
                    @click="toggleAccount(account.id)"
                >
                    <span class="inline-flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-foreground bg-card shadow-2xs">
                        <img
        ...
        <div v-else-if="step === 'schedule'" class="space-y-6">
            <div class="space-y-2">
                <Label class="text-sm font-bold">{{ $t('posts.bulk.steps.days_label') }}</Label>
                <MultiDateCalendar v-model:selected-dates="selectedDays" />
                <p v-if="selectedDays.length" class="text-xs text-foreground/70">
                    {{ $t('posts.bulk.steps.days_count', { count: String(selectedDays.length) }) }}
                </p>
            </div>

            <div class="space-y-2">
                <Label class="text-sm font-bold">{{ $t('posts.bulk.steps.times_label') }}</Label>
                ...
                <div class="flex items-center gap-2 pt-2">
                    ...
                    />
                    <Button type="button" variant="outline" dusk="bulk-add-time" @click="addTime">
            ...
            <div class="rounded-xl border-2 border-foreground bg-amber-50 p-3.5 text-sm">
                <p class="font-bold text-foreground">
                    {{ $t('posts.bulk.summary.will_create', { count: String(postsToCreate) }) }}
                </p>
                ...
                <p v-if="discardedMedia > 0" class="mt-1 text-xs font-semibold text-rose-700">
                    ⚠ {{ $t('posts.bulk.summary.discarded', { count: String(discardedMedia) }) }}
                </p>
                <p v-else-if="emptySlots > 0" class="mt-1 text-xs font-semibold text-foreground/60">
                    {{ $t('posts.bulk.summary.empty_slots', { count: String(emptySlots) }) }}
                </p>
            </div>
        </div>
        ...
        <div v-else-if="step === 'prompt'" class="space-y-4">
            <div class="space-y-2">
                <Label for="bulk-prompt" class="text-sm font-bold">{{ $t('posts.bulk.steps.prompt_label') }}</Label>
                <Textarea
                    id="bulk-prompt"
                    v-model="promptText"
                    :placeholder="$t('posts.bulk.steps.prompt_placeholder')"
                    class="min-h-[160px] resize-none"
                    dusk="bulk-prompt"
                />
                <p class="text-xs text-foreground/60">{{ $t('posts.bulk.steps.prompt_hint') }}</p>
            </div>

            <div>DEBUG SIGNATURES: {{ signatures }}</div>
            <div class="space-y-2" v-if="signatures && signatures.length > 0">
                <Label class="text-sm font-bold">Assinatura (Opcional)</Label>
                <Select v-model="selectedSignatureId">
                    <SelectTrigger dusk="bulk-signature-trigger">
                        <SelectValue placeholder="Selecione uma assinatura..." />
                    </SelectTrigger>
                    <SelectContent>
                        <SelectItem value="none">Nenhuma assinatura</SelectItem>
                        <SelectItem v-for="sig in signatures" :key="sig.id" :value="String(sig.id)">
                            {{ sig.name }}
                        </SelectItem>
                    </SelectContent>
                </Select>
        ...
        <div v-else-if="step === 'confirm'" class="space-y-4">
            <div class="rounded-xl border-2 border-foreground bg-card p-4 shadow-2xs">
            ...
            <div class="max-h-80 space-y-2 overflow-y-auto rounded-xl border-2 border-foreground bg-card p-3">
                <div
                    v-for="(row, i) in distributionPreview"
                    :key="row.media.id"
                    class="flex items-center gap-3 rounded-lg border border-foreground/15 p-2"
                >
                    <span class="inline-flex size-6 shrink-0 items-center justify-center rounded-full border-2 border-foreground bg-card text-xs font-bold text-foreground">
                        {{ i + 1 }}
                    </span>
                    <div class="size-12 shrink-0 overflow-hidden rounded-md border-2 border-foreground bg-muted">
                        <video
                            ...
                            muted
                        />
                        <img v-else :src="row.media.url" :alt="''" class="size-full object-cover" />
                    </div>
                    <p class="text-xs font-semibold text-foreground">{{ row.slot }}</p>
                </div>
            </div>
        </div>

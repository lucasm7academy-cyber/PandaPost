...
<template>
    <PopupLayout :title="$t('accounts.youtube.title')">
        ...
        <form ref="formRef" :action="selectChannel.url()" method="POST" class="hidden">
        ...
        <div class="flex flex-col gap-6">
            <div class="flex items-center gap-3">
                <img src="/images/accounts/youtube.png" alt="YouTube" class="h-10 w-10" />
                <div>
                    <h1 class="text-xl font-bold tracking-tight">{{ $t('accounts.youtube.title') }}</h1>
                    <p class="text-sm text-muted-foreground">{{ $t('accounts.youtube.description') }}</p>
                </div>
            </div>
            ...
            <div v-if="channels.length === 0 && !error" class="text-center py-12">
                ...
                <h3 class="mt-4 text-lg font-semibold">{{ $t('accounts.youtube.no_channels') }}</h3>
                ...
                >
                    <div class="flex items-center gap-4">
                        <Avatar class="h-12 w-12 rounded-lg">
                            ...
                            <AvatarFallback class="rounded-lg bg-red-100 dark:bg-red-900">
                                <IconBrandYoutube class="h-6 w-6 text-red-600 dark:text-red-400" />
                            </AvatarFallback>
                        </Avatar>

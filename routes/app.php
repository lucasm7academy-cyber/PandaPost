...
use App\Http\Controllers\Auth\GoogleDriveController;
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('connect/google-drive', [GoogleDriveController::class, 'connect'])->name('app.social.google-drive.connect');
    Route::get('accounts/google-drive/callback', [GoogleDriveController::class, 'callback'])->name('app.social.google-drive.callback');
...
Route::middleware(['auth', EnsureAccountReady::class])->group(function () {
    ...
    Route::get('workspace/{workspace}/google-drive-folders', [GoogleDriveFolderController::class, 'index'])
        ->name('app.workspace.google-drive-folders.index');
    Route::post('workspace/{workspace}/google-drive-folders', [GoogleDriveFolderController::class, 'store'])
        ->name('app.workspace.google-drive-folders.store');
    Route::patch('workspace/{workspace}/google-drive-folders/{folder}/toggle', [GoogleDriveFolderController::class, 'toggle'])
        ->name('app.workspace.google-drive-folders.toggle');
    Route::delete('workspace/{workspace}/google-drive-folders/{folder}', [GoogleDriveFolderController::class, 'destroy'])
        ->name('app.workspace.google-drive-folders.destroy');
    Route::get('workspace/{workspace}/google-drive-folders/{folder}/files', [GoogleDriveFolderController::class, 'files'])
        ->name('app.workspace.google-drive-folders.files');
    ...
    Route::post('posts/bulk', [BulkScheduleController::class, 'start'])->name('app.posts.bulk.start');
    Route::get('posts/bulk/{bulkSchedule}/loading', [BulkScheduleController::class, 'loading'])
        ->name('app.posts.bulk.loading')
        ->whereUuid('bulkSchedule');
    ...
    Route::get('assets/google-drive/list', [AssetController::class, 'listGoogleDrive'])->name('app.assets.google-drive.list');
    Route::post('assets/google-drive/download', [AssetController::class, 'downloadGoogleDrive'])->name('app.assets.google-drive.download');
...
Route::middleware(['auth'])->group(function () {
    ...
    // Invite requests (visible to any authenticated user — pending invites for their email)
    Route::get('settings/invites', [WorkspaceInviteController::class, 'requests'])->name('app.invites.requests');
});

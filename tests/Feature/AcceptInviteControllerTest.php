...
test('accept invite overwrites current_workspace_id from the previous account', function () {
    // User starts with their own account and a workspace from that account.
    $previousAccount = Account::factory()->create();
    $user = User::factory()->create([
        'email' => 'invitee@example.com',
        'account_id' => $previousAccount->id,
    ]);
    $previousAccount->update(['owner_id' => $user->id]);
    $previousWorkspace = Workspace::factory()->create([
        'account_id' => $previousAccount->id,
        'user_id' => $user->id,
    ]);
    $user->update(['current_workspace_id' => $previousWorkspace->id]);
    ...
    // After accepting, current_workspace_id MUST point to the new workspace,
    // otherwise WorkspacePolicy::view fails on the redirect target.
    $user->refresh();
    expect($user->account_id)->toBe($this->account->id);
    expect($user->current_workspace_id)->toBe($this->workspace->id);
});
